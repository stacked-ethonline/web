...
export default function Home() {
  ...
  useEffect(() => {
    const getInitialValue = async () => {
      try {
        ...
        setValue(JSON.stringify(res.state));
      } catch (e) {
  ...
  const handleAction = async (actionName: string) => {
    try {
      setSubmitting(true);
      const res = await submit(actionName, { price, tokenId: 1, timestamp: Math.floor(Date.now() / 1000) });
      if (!res) {
        throw new Error("Failed to submit action");
      }
      setValue(JSON.stringify(res.logs[0].value));
    } catch (e) {
      alert((e as Error).message);
      console.error(e);
    } finally {
    ...
    return (
      <div className="flex gap-4">
        <Button
          ...
          onClick={() => handleAction("createOrder")}
        >
          Increment
        </Button>
